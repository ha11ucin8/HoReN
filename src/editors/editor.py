...
from ..util.inference_counter import (
    get_generation_call_count,
    reset_generation_call_count,
)
from ..util.edit_timer import reset_edit_timer, get_edit_time
...
class BaseEditor:
    ...
    def __init__(self, hparams: HyperParams):
        ...
        if type(self.model_name) is str:
            ...
            torch_dtype = torch.bfloat16 if getattr(hparams, "bf16", False) else torch.float32
            ...
            if "t5" in self.model_name.lower():
                ...
                self.model = AutoModelForCausalLM.from_pretrained(
                    self.model_name, **model_kwargs, trust_remote_code=True
                )
                ...
                self.model = AutoModelForCausalLM.from_pretrained(
                    self.model_name, fp32=False, trust_remote_code=True, **model_kwargs
                )
            ...
            if (
                ...
                and (
                    ...
                    or isinstance(self.tok, LlamaTokenizerFast)
                    or isinstance(self.tok, PreTrainedTokenizerFast)
                )
                and (hparams.alg_name not in ["ROME", "MEMIT", "EMMET", "R-ROME", "AlphaEdit", "CORE"])
            ):
            ...
            if (
                ...
                and (hparams.alg_name in ["ROME", "MEMIT", "EMMET", "R-ROME", "AlphaEdit", "CORE"])
            ):
    ...
    def batch_edit(
        ...
        **kwargs,
        ...
        assert BatchEditor.is_batchable_method(
            self.alg_name
        ), f"The Method {self.alg_name} can not batch edit examples."
        ...
        for record_chunks in _chunks(requests, self.hparams.batch_size):
            ...
            edited_model, weights_copy = self.apply_algo(
                self.model, self.tok, record_chunks, self.hparams, copy=False, return_orig_weights=True
            )
            ...
            if sequential_edit:
                ...
                if (
                    self.alg_name == "KN"
                    or self.alg_name == "GRACE"
                    or self.alg_name == "WISE"
                    or self.alg_name == "HOREN"
                ):
                    with torch.no_grad():
                        weights_copy()
            ...
            for i, request in enumerate(record_chunks):
                chunk_metrics[i]["pre"] = compute_edit_quality(
                    self.model,
                    self.model_name,
                    self.hparams,
                    self.tok,
                ...
                if "locality" in chunk_metrics[i]["post"].keys():
                    for locality_key in request["locality"].keys():
                        ...
                        if hasattr(self.hparams, "evaluation_type") and self.hparams.evaluation_type == "LLM-judge":
                            locality_result.append(
                                float(
                                    chunk_metrics[i]["post"]["locality"][f"{locality_key}_output"]
                                    == chunk_metrics[i]["pre"]["locality"][f"{locality_key}_output"]
                                )
                            )
                        else:
                            for ans, label in zip(
                                chunk_metrics[i]["post"]["locality"][f"{locality_key}_output"],
                                chunk_metrics[i]["pre"]["locality"][f"{locality_key}_output"],
                            ):
                                n = min(len(ans), len(label))
                                m = max(len(ans), len(label))
                                matches = int(np.sum(np.equal(ans[:n], label[:n]))) if m > 0 else 0
                                locality_result.append(matches / m if m > 0 else 1.0)
    ...
    def edit_requests(
        ...
        **kwargs,
        ...
        def edit_evaluation(all_metrics, request, edited_model, idx, test_generation, icl_examples, **kwargs):
                ...
                if "metric_kwargs" in kwargs:
                    all_metrics[idx].update(
                        compute_sent_metric(
                            ...
                            self.hparams,
                            self.tok,
                            metric_kwargs=kwargs["metric_kwargs"][idx],
                ...
                if "locality" in all_metrics[idx]["post"].keys() and not hasattr(self.hparams, "evaluation_type"):
                    for locality_key in request["locality"].keys():
                        ...
                        if hasattr(self.hparams, "evaluation_type"):
                            locality_result.append(
                                float(
                                    all_metrics[idx]["post"]["locality"][f"{locality_key}_output"]
            ...
            return get_generation_call_count()

        def count_editor_params(weights_copy, edited_model):
            """
            Estimate editor-side parameter count (not full model params).
            Prefer the explicit edited-weight set when available.
            """
            if isinstance(weights_copy, dict) and len(weights_copy) > 0:
                return int(sum(v.numel() for v in weights_copy.values()))

            # Fallback for adapter-style editors where edited params are trainable modules.
            return int(sum(p.numel() for p in edited_model.parameters() if p.requires_grad))
            ...
            for batch_start in tqdm(range(0, len(requests), batch_size), total=total_batches):
                ...
                if edited_count in custom_metric_periods or edited_count % metric_period == 0:
                    ...
                    for j, request in enumerate(tqdm(requests[:edited_count], total=len(requests[:edited_count]))):
                        ...
                        checkpoint_generate_calls += gen_calls
                    checkpoint_inference_time = time() - inference_start
                    avg_inference_time = (
                        checkpoint_inference_time / checkpoint_generate_calls if checkpoint_generate_calls > 0 else 0.0
                    )
                    ...
                    summary_metrics_(
                        ...
                        num_of_editor_params=num_of_editor_params,
                        log_file_prefix=log_file_prefix,
                    )
        ...
        if not sequential_edit and (
            not hasattr(self.hparams, "evaluation_type") or self.hparams.evaluation_type != "generate-text"
        ):
        ...
        **kwargs,
        ...
        if "pre_edit" in kwargs and kwargs["pre_edit"] is not None:
            ...
            for i, request in enumerate(tqdm(requests)):
                results = {}
                results["pre"] = {}
                results["pre"]["rewrite_ans"] = text_generate(
                    self.model,
                    self.model_name,
                    self.hparams,
                    self.tok,
                ...
                results["pre"]["rephrase_ans"] = text_generate(
                    self.model,
                    self.model_name,
                    self.hparams,
                    self.tok,
                    request["rephrase_prompt"],
                    self.hparams.device,
                    eval_metric=eval_metric,
                    test_generation=test_generation,
                )
        ...
        def post_edit_results(
            all_results, request, edited_model, idx, eval_metric, test_generation, icl_examples, **kwargs
        ):
            if self.alg_name == "IKE":
                ...
                results_post = {}
                results_post["rewrite_ans"] = text_generate(
                    edited_model,
                    self.model_name,
                    self.hparams,
                    self.tok,
                ...
                results_post["rephrase_ans"] = text_generate(
                    edited_model,
                    self.model_name,
                    self.hparams,
                    self.tok,
                ...
                if test_generation:
                    if self.hparams.alg_name == "GRACE" or self.hparams.alg_name == "HOREN":
                        results_post["fluency"] = test_generation_quality(
                            model=edited_model,
                            tok=self.tok,
                            prefixes=(
                                request["prompt"]
                ...
                all_results[idx].update({"case_id": idx, "requested_rewrite": request, "post": results_post})
            if verbose:
                LOG.info(f"{idx} editing: {request['prompt']} -> {request['target_new']}")
        ...
        if sequential_edit:
            ...
            for i, request in enumerate(requests):
                post_edit_results(
                    all_results, request, edited_model, i, eval_metric, test_generation, icl_examples, **kwargs
                )
        else:
            for i, request in enumerate(tqdm(requests, total=len(requests))):
                edited_model, weights_copy, icl_examples = edit_func(request)
