...
class AKEWUnifiedDataset(Dataset):
    ...
    def __init__(
        self,
        data_dir: str,
        dataset_type: str = "wikiupdate",
        size: typing.Optional[int] = None,
        config=None,
        model_name: str = None,
        use_unstructured_data: bool = False,
        *args,
        **kwargs,
    ):
    ...
    def _process_counterfact_unstructured(self, record, case_id):
        if "Llama3-8B-Instruct" in self.model_name:
            return {
                ...
                "sub_question": get_list_llama_without_answer(
                    [q["prompt"].format(q["subject"]) for q in record["requested_rewrite"]["unsfact_triplets_GPT"]][:5],
                    False,
                ),
            ...
            return {
                ...
                "sub_question": get_list_qwen_without_answer(
                    [q["prompt"].format(q["subject"]) for q in record["requested_rewrite"]["unsfact_triplets_GPT"]][:5],
                    False,
                ),
                "sub_answer": [q["target"] for q in record["requested_rewrite"]["unsfact_triplets_GPT"]][:5],
            ...
            return {
                ...
                "sub_question": [
                    q["prompt"].format(q["subject"]) for q in record["requested_rewrite"]["unsfact_triplets_GPT"]
                ][:5],
                "sub_answer": [q["target"] for q in record["requested_rewrite"]["unsfact_triplets_GPT"]][:5],
            }
    ...
    def _process_mquake_unstructured(self, record, case_id):
        ...
        if "Llama3-8B-Instruct" in self.model_name:
            return {
                ...
                "sub_question": get_list_llama_without_answer(
                    [q["prompt"].format(q["subject"]) for q in record["requested_rewrite"][0]["unsfact_triplets_GPT"]][
                        :5
                    ],
                    False,
                ),
    ...
    def collate_fn(self, batch):
        ...
        loc_ans = [
            l[0]["ground_truth"][0] if isinstance(l[0]["ground_truth"][0], str) else l[0]["ground_truth"][0][0]
            for l in loc_data
        ]
    ...
    def collate_gpt_fn(self, batch):
        ...
        loc_ans = [
            l[0]["ground_truth"] if isinstance(l[0]["ground_truth"][0], str) else l[0]["ground_truth"][0]
            for l in loc_data
        ]
