...
import regex
import string
from ..util.inference_counter import increment_generation_call_count
...
def test_prediction_acc_LLM_judge(model, tok, hparams, prompts, targets, device, locality=False):
    ...
    for prompt in prompts:
        ...
        # add a template
        increment_generation_call_count()
        ...
        if isinstance(model, T5ForConditionalGeneration):
            ...
            trunc_gen_tokens = gen_tokens[0][
                prompt_tok["input_ids"].shape[1] :
            ]  # decoder-only model provied generated content containing prompt
        # if locality:
    ...
    if vanilla_generation:
        ...
        for prompt, target_new in zip(prompts, targets):
            ...
            if locality:
                ...
                acc = (
                    np.mean(np.equal(target_new_tokens, ans_tokens_list))
                    if len(target_new_tokens) == len(ans_tokens_list)
                    else 0
                )
                if math.isnan(acc):
                    acc = 1
...
def per_generation(model, tok, max_out_len: int, target_per, device, edited_model=None, IKE=False, **kwargs):
...
def F1(model, tok, hparams, prompts, targets, device, locality=False, vanilla_generation=True):
    if vanilla_generation:
        ...
        return f1_score(
            target_new_tokens, gen_token.detach().cpu().numpy().tolist()[0][-len(target_new_tokens) :], average="macro"
        )
    if isinstance(prompts, str):
        prompts, targets = [
            prompts,
...
def test_instance_change(model, tok, max_length, prompts, targets, device, P=None):
    ...
    with torch.no_grad():
        ...
        model_response = [
            tok.decode(x, skip_special_tokens=True) for x in pre_edit_outputs.detach().cpu().numpy().tolist()
        ]
...
def test_safety_gen(model, tokenizer, test_prompt, cuda, max_tokens=1624, max_output_tokens=600):
