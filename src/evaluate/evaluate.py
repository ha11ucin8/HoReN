...
def compute_edit_quality(
    ...
    ret = compute_rewrite_or_rephrase_quality(
        model, model_name, hparams, tok, rewrite_prompts, target_new, device=device, eval_metric=eval_metric
    )
...
def compute_rewrite_or_rephrase_quality(
    ...
    if hasattr(hparams, "evaluation_type") and hparams.evaluation_type == "LLM-judge":
        acc, gen_content = test_prediction_acc_LLM_judge(
            model, tok, hparams, prompt, target_new, device, locality=False
        )
        ret = {f"{key}_acc": acc, f"{key}_gen_content": gen_content}
    elif hasattr(hparams, "evaluation_type") and hparams.evaluation_type == "generate-text":
        gen_content_model = test_prediction_acc_LLM_judge(
            model, tok, hparams, prompt, target_new, device, locality=False
        )
        ret = {f"{key}_gen_content": gen_content_model}
...
def compute_locality_quality(
    ...
    if hasattr(hparams, "evaluation_type"):
        loc_tokens = test_prediction_acc_LLM_judge(
            model, tok, hparams, prompt, locality_ground_truth, device, locality=True
        )
    else:  # traditional evaluation
        if "t5" in model_name.lower():
            loc_tokens = test_seq2seq_batch_prediction_acc(
                model, tok, hparams, prompt, locality_ground_truth, device, locality=True
            )
        else:
            loc_tokens = test_prediction_acc(
                model,
                tok,
                hparams,
                prompt,
    ...
    ret = {f"{locality_key}_output": loc_tokens}
    return ret
...
def compute_portability_quality(
    ...
    # using real-world evaluation: autoregressive decoding, natural stop criteria, LLM-as-a-Judge
    if hasattr(hparams, "evaluation_type") and hparams.evaluation_type == "LLM-judge":
        portability_correct = test_prediction_acc_LLM_judge(
...
def compute_icl_edit_quality(
    ...
    if pre_edit:
        ...
        edit_acc = icl_lm_eval(model, model_name, hparams, tok, icl_examples, target_new, new_fact)
    ret = {f"rewrite_acc": [edit_acc]}
    ret["locality"] = {}
    ret["portability"] = {}
    if rephrase is not None:
        rephrase_acc = icl_lm_eval(
            model,
            model_name,
            hparams,
            tok,
    ...
    if "locality" in record.keys() and any(record["locality"]):
        for locality_key in record["locality"].keys():
            if isinstance(record["locality"][locality_key]["ground_truth"], list):
                ...
                post_neighbor = []
                for x_a, x_p in zip(
                    record["locality"][locality_key]["ground_truth"], record["locality"][locality_key]["prompt"]
                ):
                    tmp_pre_neighbor = icl_lm_eval(
                        model, model_name, hparams, tok, [""], x_a, f"{x_p}", neighborhood=True
                    )
                    tmp_post_neighbor = icl_lm_eval(
                        model,
                        model_name,
                        hparams,
                        tok,
        ...
        for portability_key in record["portability"].keys():
            ...
            if isinstance(record["portability"][portability_key]["ground_truth"], list):
                ...
                for x_a, x_p in zip(
                    ...
                    tmp_portability_acc = icl_lm_eval(
                        model, model_name, hparams, tok, icl_input, x_a, f"{x_prefix}{x_p}"
                    )
                portability_acc.append(tmp_portability_acc)
            else:
                portability_acc = icl_lm_eval(
                    model,
                    model_name,
                    hparams,
                    tok,
...
def icl_lm_eval(
    model, model_name, hparams: HyperParams, tokenizer, icl_examples, target, x, neighborhood=False
) -> typing.Dict:
